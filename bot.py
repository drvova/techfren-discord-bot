...
import config  # Bot configuration
...
async def on_message(message):
    ...
    if not message.author.bot and message_contains_gif(message):
        ...
        if not can_post_gif:
            ...
            expired_users = [
                uid for uid, expiry in _gif_warned_users.items() if expiry <= now
            ]
            ...
            if user_warning_expiry is None or user_warning_expiry <= now:
                ...
                try:
                    ...
                    logger.debug(
                        f"User {user_id} warned about GIF limit, will suppress warnings until {_gif_warned_users[user_id]}"
                    )
                except discord.Forbidden:
                    logger.warning(
                ...
                logger.debug(
                    f"User {user_id} already warned, silently deleting GIF without additional warning"
                )
    ...
    try:
        ...
        # Note: Automatic image summarization removed - vision model analyzes images on-demand
        # when users query about them, eliminating redundant API calls
        image_summary = None
...
async def _handle_slash_command_wrapper(
    ...
    try:
        ...
        if e.code == 10062:
            ...
            logger.error(
                f"Interaction expired for {command_name} - took too long to respond"
            )
        ...
        if e.status == 400 and e.code == 40060:
            ...
            logger.warning(
                f"Interaction already acknowledged for {command_name}, continuing..."
            )
        else:
    ...
    try:
        ...
        await handle_summary_command(
            context, response_sender, thread_manager, hours=hours, bot_user=bot.user
        )
