...
async def generate_image_summary(image_url: str) -> Optional[str]:
    ...
    try:
        ...
        logger.error(
            f"Error generating image summary for {image_url}: {str(e)}", exc_info=True
        )
        return None
...
async def call_llm_api(query, message_context=None):
    ...
    try:
        ...
        image_data_urls = (
            await get_all_images_from_context(message_context)
            if message_context
            else []
        )
        ...
        if image_data_urls:
            ...
            for idx, image_url in enumerate(image_data_urls):
                # Log first 100 chars of data URL to verify format
                url_preview = image_url[:100] if len(image_url) > 100 else image_url
                logger.debug(f"Image {idx + 1} data URL preview: {url_preview}...")

                user_message_content.append(
                    {"type": "image_url", "image_url": {"url": image_url}}
                )
            logger.info(
                f"Vision mode enabled: sending {len(image_data_urls)} image(s) to LLM"
            )
            logger.debug(
                f"User message content structure: {len(user_message_content)} parts (1 text + {len(image_data_urls)} images)"
            )
        else:
            # Text-only mode
            user_message_content = user_content
            logger.debug("Text-only mode: no images attached")
...
async def call_llm_for_summary(messages, channel_name, date, hours=24):
    ...
    try:
        ...
        max_input_length = (
            60000  # ~15k tokens for input, allowing room for system prompt and output
        )
...
"""
        ...
        if hasattr(completion, "citations") and completion.citations:
            logger.info(
                f"Found {len(completion.citations)} citations from Perplexity for summary"
            )
            citations = completion.citations
